...
export function ProductsPagination(props: ProductsPaginationProps) {
				...
				<li key={index}>
					<Button
						...
						className={`w-8 h-8  sm:w-[3.75rem] sm:h-[3.75rem] !px-0 !py-0 rounded-lg 
                            ${index === currentPage ? "" : "bg-secondary-bg-color text-black"}`}
			...
			buttons.push(
				<li key={pages}>
					<Button
						id={pages.toString()}
						className={`w-8 h-8 sm:w-[3.75rem] sm:h-[3.75rem] !px-0 !py-0 rounded-lg 
                    ${pages === currentPage ? "" : "bg-secondary-bg-color text-black"}`}
						onClick={handlePaginationClick}
					>
						{pages}
					</Button>
			...
			buttons.unshift(
				<li key={1}>
					<Button
						id={"1"}
						className={`w-8 h-8 sm:w-[3.75rem] sm:h-[3.75rem] !px-0 !py-0 rounded-lg 
                    ${1 === currentPage ? "" : "bg-secondary-bg-color text-black"}`}
						onClick={handlePaginationClick}
					>
						{1}
					</Button>
	...
	const paginationMarkup = () => {
		if (productsPagination === "numeric") {
			return <ul className="space-x-4 md:space-x-[2.375rem] flex items-center justify-center">{numericPaginationMarkup()}</ul>;
		} else {
			return (
				<Button
	...
	return <div className={`text-center mb-16 ${productsShown < products.length ? "mt-8" : ""}`}>{productsShown < products.length && paginationMarkup()}</div>;
}
